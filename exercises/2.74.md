...
We have chosen to design the system additive in respect to new divisions (new
data types), so integrating with a new company requires only local changes.

The new division must tag its personnel files and install implementations of the
generic operations (`get-record`, `get-salary`) expected by headquarters and
conforming to the existing headquarters interface.

No changes are required to the headquarters code or to existing divisions,
preserving both modularity and autonomy.
