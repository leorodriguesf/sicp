...
Note that this implementation does not work when we have a mixed of operation defined in the operations table. For example:
...
would not work because we would look for a procedure defined for either arguments as is `(rational complex)`, `(complex complex)` or `(rational rational)`, but not for a procedure with types mixed.